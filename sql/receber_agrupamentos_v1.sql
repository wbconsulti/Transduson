...
(
    SELECT
        "Ano", "MesNum", "Cod.Cliente", "Nome Cliente", "Vencimento", "Status Previsão", "Valor Ano",
        "Valor Mes", "Valor Variavel", "Lancado (NF)", "A RECEBER", "Recebido", "Data Receb.", "Parcela", "Var Realiz",
        "CFO", "Nome CFO", "Tipo CFO", "Raiz CFO", "CCusto", "Nome CCusto", "DocNumPed", "LinhaPed", "Usage",
        "Centro Custo", "Class.Financ.", "DocNumSAP", "InvType", "ContaRecebimento", "Comments"
    FROM Detalhe
)
...
-- ===================================================================
SELECT
    10 AS "_ord", 'Geral' AS "Agrupamento", CAST(NULL AS INTEGER) AS "Ano", CAST(NULL AS INTEGER) AS "MesNum", CAST(NULL AS DATE) AS "Vencimento", CAST('TOTAL' AS NVARCHAR(50)) AS "Status Previsão", CAST(NULL AS NVARCHAR(30)) AS "Cod.Cliente",
    CAST('TOTAL GERAL' AS NVARCHAR(200)) AS "Nome Cliente",
    CAST(SUM("Valor Ano") AS DECIMAL(19,6)) AS "Valor Ano", CAST(SUM(CASE WHEN COALESCE("Valor Mes", 0) = 0 THEN COALESCE("Valor Variavel", 0) ELSE COALESCE("Valor Mes", 0) END) AS DECIMAL(19,6)) AS "Valor Mes", CAST(SUM("Valor Variavel") AS DECIMAL(19,6)) AS "Valor Variavel", CAST(SUM("Lancado (NF)") AS DECIMAL(19,6)) AS "Lancado (NF)", CAST(SUM("A RECEBER") AS DECIMAL(19,6)) AS "A RECEBER", CAST(SUM("Recebido") AS DECIMAL(19,6)) AS "Recebido", CAST(SUM("Var Realiz") AS DECIMAL(19,6)) AS "Var Realiz"
FROM Resultado
...
SELECT
    ...
    CAST(SUM("Valor Ano") AS DECIMAL(19,6)), CAST(SUM(CASE WHEN COALESCE("Valor Mes", 0) = 0 THEN COALESCE("Valor Variavel", 0) ELSE COALESCE("Valor Mes", 0) END) AS DECIMAL(19,6)), CAST(SUM("Valor Variavel") AS DECIMAL(19,6)), CAST(SUM("Lancado (NF)") AS DECIMAL(19,6)), CAST(SUM("A RECEBER") AS DECIMAL(19,6)), CAST(SUM("Recebido") AS DECIMAL(19,6)), CAST(SUM("Var Realiz") AS DECIMAL(19,6))
